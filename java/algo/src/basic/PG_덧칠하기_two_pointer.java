...
public class PG_덧칠하기_two_pointer {
    ...
    int[] paint(int m, List<Integer> list) {
        int start, end, maxIdx, maxCnt;
        start = end = 0;
        maxIdx = maxCnt = -1;
        int size = list.size();
        while (start<=end&&end<size) {
            int tmp = list.get(end) - list.get(start) + 1;
            if (tmp > m) start++;
            else {
