...
public class ProductTest {
	...
	public static void main(String[] args) {
		...
		pm.addReview(new Review(1, "R00001", "user", "content"));
		...
		try {
			for(Product p : pm.searchByName("M")) {
		...
		System.out.println("********************S 냉장고의 댓글 보기********************");
		try {
			for(Review r : pm.getProductReview("R00002")) {
				System.out.println(r);
			}
		...
		try {
			pm.sell("R00001", 100); // 수량 부족
			pm.sell("R0000", 100); // 상품 존재 X
		} catch (ProductCodeNotFoundException | QuantityException e) {
			e.printStackTrace();
		}
